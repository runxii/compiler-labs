...
CmpInstruction::CmpInstruction(unsigned opcode, Operand *dst, Operand *src1, Operand *src2, BasicBlock *insert_bb): Instruction(CMP, insert_bb){
    ...
    src1->addUse(this);
    src2->addUse(this);
    dst->getType()->kind=src1->getType()->kind;
}
...
MultiAllocaInstruction::MultiAllocaInstruction(Operand *src, Operand *dst, SymbolEntry *se, BasicBlock *insert_bb) : Instruction(ALLOCA, insert_bb)
{
    operands.push_back(dst);
    dst->setDef(this);
    this->se = se;
...
MultiAllocaInstruction::~MultiAllocaInstruction()
{
    operands[0]->setDef(nullptr);
    if(operands[0]->usersNum() == 0)
        delete operands[0];
...
void MultiAllocaInstruction::output() const
{
    std::string dst, type;
    dst = operands[0]->toStr();
    type = se->getType()->toStr();
...
TruncInstruction::TruncInstruction ( Operand *dst, Operand *src,BasicBlock *insert_bb) : Instruction(SEXT,insert_bb)
{
    operands.push_back(dst);
    operands.push_back(src);
    dst->setDef(this);
...
void TruncInstruction ::output() const
{    
    Operand* dst=operands[0];
    Operand* src=operands[1];
    fprintf(yyout,"  %s = trunc i32 %s to i1\n",dst->toStr().c_str(),src->toStr().c_str() );
}
