...
protected:
    ...
    enum {BINARY, COND, UNCOND, RET, LOAD, STORE, CMP, ALLOCA, CALL, XOR, ZEXT, SEXT};
};
...
class MultiAllocaInstruction : public Instruction
{
public:
    Operand *src;
    MultiAllocaInstruction(Operand *src, Operand *dst, SymbolEntry *se, BasicBlock *insert_bb = nullptr);
    ~MultiAllocaInstruction();
    void output() const;
private:
    SymbolEntry *se;
};
...
{
    public:
      TruncInstruction( Operand *dst, Operand *src,BasicBlock *insert_bb = nullptr);
      void output() const;
};
